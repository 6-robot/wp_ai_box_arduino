...
unsigned char* data_tts;
int data_tts_length = 0;
int data_tts_max = 16000 * 60 * 2;
...
void xfyun_init()
{
  ...
  data_tts = (unsigned char*)ps_malloc(data_tts_max);
  if (data_tts == NULL) {
      Serial.print("[xfyun_tts] data_tts 申请内存失败！");
  }
  // 语音合成 TTS 回调函数
  clientTTS.onMessage([&](WebsocketsMessage message) {
    ...
    //Serial.printf("[xfyun_tts]response_len = %d  \n", response_len);

    data_tts_length = decode_base64((unsigned char*)response, response_len, data_tts);
    size_t bytes_written = 0;
    i2s_write(I2S_NUM_1, (uint16_t*)data_tts, data_tts_length, &bytes_written, portMAX_DELAY);

    if (responseJson["data"]["status"].as<int>() == 2) {  //收到结束标志
      Serial.println("Playing complete.");
      delay(500);
      i2s_zero_dma_buffer(I2S_NUM_1);  // 清空I2S DMA缓冲区
    }
  });
...
{
  String TTSurl = xfyun_url(apiSecret, apiKey, "/v2/tts", "ws-api.xfyun.cn");
  ...
  String TTStextbase64 = base64::encode(text);
  DynamicJsonDocument requestJson(51200);
  String app_id = appID;
  String voice_name = voiceName;
  requestJson["common"]["app_id"] = app_id;
  requestJson["business"]["aue"] = "raw";
  requestJson["business"]["vcn"] = voice_name;
